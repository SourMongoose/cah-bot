...
class Shard:
    def __init__(self, shard, client):
        # SQL setup
    ...
    async def play(self, ch, p, s):
        ...
        try:
            ...
            for x in s:
                ...
                # check for blank
                if card == "":
                    await self.client.send_message(p, "You tried to send a blank card without filling it in first!\nMessage me what you'd like to put in the blank.")
                    return
                newMid.append(card)
    ...
    async def displayMid(self, ch):
        ...
        try:
            ...
            await self.client.send_message(ch, "Encountered error while displaying - answer selection may not function normally. If the czar is unable to select, try using `"+c+"!display`.")
            return
            ...
            for i in range(config.C[ch]["nPlayers"]-1):
                try:
                    ...
                    if not error_occured:
                        error_occured = True
                        await self.client.send_message(ch, "An error occurred while adding a letter; if the letter of your choice is not shown, please choose it by adding the letter manually.")
    ...
    async def on_message(self, message):
        ...
        if self.shard == 0:
            if ch.is_private:
                if au in config.P: # check that user has a blank card
                    ...
                    self.C.execute("""insert into Messages values (?, ?)""", (au.id, edited_msg))
                    self.conn.commit()
        ...
        # invite link
        if msg == c+"!invite":
            await self.client.send_message(ch, "Use this link to invite the bot to your own server:\n<https://discordapp.com/api/oauth2/authorize?client_id=429024440060215296&permissions=134294592&scope=bot>")
        
        # vote link
        if msg == c+"!vote":
            await self.client.send_message(ch, "Use this link to vote for the bot on discordbots.org:\n<https://discordbots.org/bot/429024440060215296/vote>")
                        ...
                        for i in range(len(config.C[ch]["players"])):
                            ...
                            if mt == p.mention:
                                if not config.C[ch]["played"][i]:
                                    ...
                                    await self.client.send_message(ch, au.mention + " has voted to kick " + p.mention + ". " + str(cnt) + "/" + str(config.C[ch]["nPlayers"]-1) + " votes needed")
